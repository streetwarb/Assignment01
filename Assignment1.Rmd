...
set.seed(1234)
p.norm <- rnorm(n=pop.size, mean=pop.mean, sd=pop.sd) #generating population
dat <-data.frame()   
for (i in 1:num.of.samples){
  s.norm <- sample(p.norm, size=sample.size, replace=FALSE) 
  s.mean <- mean(s.norm)
  s.sd <-sd(s.norm)
  data.temp <-data.frame(s.mean, s.sd) 
  dat <- rbind(dat, data.temp)
}

library("ggplot2")
#sample statistics value vs theoretical values
scatterplot0 <- ggplot(data=dat, aes(dat$s.mean, dat$s.sd)) + geom_point() + geom_point(aes(100, 15), color='red', size =3) + labs(title ="Sample statistics Vs Theoretical values", x="Sample Mean", y= "Sample Sd")
scatterplot0

#sample means distribution
scatterplot1 <- ggplot(data=dat, aes(1:200, dat$s.mean)) + geom_point() + geom_hline(yintercept = 100, color="red") + labs(title = "Sample Means distribution", x="Sample Sets", y ="Sample Mean")
scatterplot1

#sample sd distribution
scatterplot2 <- ggplot(data=dat, aes(1:200, dat$s.sd)) + geom_point() + geom_hline(yintercept = 15, color="blue") + labs(title = "Sample Sd distribution", x="Sample Sets", y ="Sample Sd")
scatterplot2

#histogram of sample means
hist.mean <- ggplot(data=dat, aes(x=dat$s.mean)) + geom_histogram(binwidth=0.3, color="red", fill="wheat", alpha=.2) + labs(title = "Histogram of Sample Means", x = "Sample Mean", y="Frequency")
hist.mean

#histogram of sample sd
hist.sd <- ggplot(data=dat, aes(x=dat$s.sd)) + geom_histogram(binwidth=0.3, color="black", fill="wheat", alpha=.2) + labs(title = "Histogram of Sample Sd", x = "Sample Sd", y="Frequency")
hist.sd
...
# put your R code here inside the blocks
ks <- 1:30
mse.train <- numeric(length(ks))
mse.test <- numeric(length(ks))
...
ggplot() + geom_line(aes(x=ks, y=mse.train), color="red") + geom_point(aes(x=ks, y=mse.train)) + geom_line(aes(x=ks, y=mse.test), color="blue") + geom_point(aes(x=ks, y=mse.test)) + scale_x_reverse(lim=c(30,1)) + theme_bw()
...
mse.opt <- data.frame(1:20, mse.opt)
ggplot(data = mse.opt, aes(1:20, mse.opt)) + geom_point(color="red") + coord_cartesian(ylim = c(0,30))
